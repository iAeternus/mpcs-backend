package com.ricky.common.exception;

import lombok.Getter;


/**
 * @author Ricky
 * @version 1.0
 * @date 2024/9/9
 * @className ErrorCodeEnum
 * @desc 错误码
 * 400：请求验证错误，请求中资源所属关系错误等
 * 401：认证错误
 * 403：权限不够
 * 404：资源未找到
 * 409：业务异常
 * 422：配置错误
 * 426：套餐检查失败
 * 500：系统错误
 */
@Getter
public enum ErrorCodeEnum {

    // 400
    PARAMS_ERROR(400),
    INVALID_EPS(400),
    INVALID_ENUM_KEY(400),
    REQUEST_VALIDATION_FAILED(400),
    UNSUPPORTED_FILE_TYPES(400),
    FILE_MUST_NOT_BE_EMPTY(400),
    FILE_ORIGINAL_NAME_MUST_NOT_BE_BLANK(400),
    HAS_SENSITIVE_WORD(400),
    INVALID_ID_PREFIX(400),
    ES_FILE_INVALID(400),

    // 401
    AUTHENTICATION_FAILED(401),

    // 403
    ACCESS_DENIED(403),

    // 404
    NOT_FOUND(404),
    AR_NOT_FOUND(404),
    AR_NOT_FOUND_ALL(404),
    DOMAIN_EVENT_NOT_FOUND(404),
    FILE_NOT_FOUND(404),
    FOLDER_NOT_FOUND(404),
    FOLDER_HIERARCHY_NOT_FOUND(404),
    FILE_EXTRA_NOT_FOUND(404),
    PUBLIC_FILE_NOT_FOUND(404),
    COMMENT_NOT_FOUND(404),
    COMMENT_HIERARCHY_NOT_FOUND(404),
    GROUP_NOT_FOUND(404),
    UPLOAD_SESSION_NOT_FOUND(404),

    // 409
    FILE_ALREADY_EXISTS(409),
    FILE_READ_FAILED(409),
    FILE_DELETE_FAILED(409),
    INVALID_HASH_ALGORITHM(409),
    UNSUPPORTED_FILE_TYPE(409),
    STORE_FILE_FAILED(409),
    MUST_SIGN_AGREEMENT(409),
    VERIFICATION_CODE_COUNT_OVERFLOW(409),
    VERIFICATION_CODE_CHECK_FAILED(409),
    USER_WITH_MOBILE_OR_EMAIL_ALREADY_EXISTS(409),
    VERIFICATION_CODE_ALREADY_SENT(409),
    TOO_MANY_VERIFICATION_CODE_FOR_TODAY(409),
    USER_ALREADY_LOCKED(409),
    UPLOAD_ALREADY_COMPLETED(409),
    SAVE_CHUNK_FAILED(409),
    MERGE_CHUNKS_FAILED(409),
    FILE_HASH_MISMATCH(409),
    FILE_NAME_DUPLICATES(409),
    FOLDER_HIERARCHY_TOO_DEEP(409),
    FOLDER_HIERARCHY_WITH_CUSTOM_ID_ALREADY_EXISTS(409),
    FOLDER_WITH_NAME_ALREADY_EXISTS(409),
    FOLDER_NAME_DUPLICATES(409),
    GENERATE_SUMMARY_FAILED(409),
    EXTRACT_FILE_TEXT_FAILED(409),
    ES_SEARCH_ERROR(409),
    MAX_GROUP_MANAGER_REACHED(409),
    GROUP_WITH_NAME_ALREADY_EXISTS(409),
    NO_MORE_THAN_ONE_VISIBLE_GROUP_LEFT(409),
    NOT_ALL_USERS_EXIST(409),
    PUBLIC_FILE_STATUS_ERROR(409),
    COMMENT_STATUS_ERROR(409),
    COMMENT_HIERARCHY_TOO_DEEP(409),
    USER_MOBILE_AND_EMAIL_BOTH_BLANK(409),
    NOT_ALL_FOLDERS_EXIST(409),
    HIERARCHY_ERROR(409),
    CANNOT_MOVE_ROOT(409),
    OSS_ERROR(409),
    CANNOT_DELETE_ROOT_FOLDER(409),
    CANNOT_MOVE_FILE_TO_ROOT(409),

    // 422
    CONFIG_ERROR(422),

    // 426

    // 429
    TOO_MANY_REQUEST(429),

    // 500
    SYSTEM_ERROR(500),
    ES_INDEX_CHECK_ERROR(500),
    ES_INDEX_CREATE_ERROR(500),
    ES_INDEX_MAPPING_ERROR(500),
    ES_DOCUMENT_INDEX_ERROR(500),
    ES_DOCUMENT_DELETE_ERROR(500),

    // Collaboration - 协同编辑
    COLLAB_SESSION_NOT_FOUND(404),
    COLLAB_SESSION_ALREADY_EXISTS(409),
    COLLAB_SESSION_FULL(409),
    COLLAB_USER_NOT_IN_SESSION(404),
    COLLAB_VERSION_CONFLICT(409),
    COLLAB_OPERATION_INVALID(400),
    COLLAB_SESSION_EXPIRED(409),
    COLLAB_OT_TRANSFORM_FAILED(500),
    ;

    /**
     * 错误码
     */
    private final int status;

    ErrorCodeEnum(int status) {
        this.status = status;
    }

}
