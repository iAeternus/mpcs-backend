...
public class MongoCachedGroupRepository extends MongoBaseRepository<Group> {
    ...
    public UserCachedGroups cachedUserAllGroups(String userId) {
        ...
        query.fields().include("name", "active", "userId", "members", "memberAuthorizations", "grants", "inheritancePolicy");
    ...
    public CachedGroup cachedById(String groupId) {
        ...
        query.fields().include("name", "active", "members", "memberAuthorizations", "grants", "inheritancePolicy", "customId");
    ...
    @Caching(evict = {@CacheEvict(value = GROUPS_CACHE, key = "'customId:' + #customId")})
    public void evictGroupCacheByCustomId(String customId) {
        requireNotBlank(customId, "Custom ID must not be blank.");

        log.debug("Evicted groups cache for customId[{}].", customId);
    }
